...
.dropdown-content {
        ...
        .form-container .file-upload-container input {
            display: none;
        }
...
<section style="background-color:  #FFFFFF; padding: 70px; display: flex; align-items: center; justify-content: center;">
    <div style="margin-left: 55px; margin-top: -35px;">
        ...
        <input name="job_title" type="text" placeholder="Judul Pekerjaan" required autofocus>
        
        <h6>Detail Pekerjaan*</h6>
        <input name="job_description" type="text" placeholder="Detail Pekerjaan" required>
        ...
        <input name="job_type" type="text" placeholder="Jenis Pekerjaan" required>
        
        <h6>Keterampilan yang diperlukan*</h6>
        <input name="required_skills" type="text" placeholder="Keterampilan yang diperlukan" required>

        <h2 style="margin-top: 75px;">Lampiran Berkas (Opsional)</h2>
        <hr style="border: 1px solid #ddd; margin-bottom: 20px;">
        <div class="file-upload-container">
            <label for="file-upload">
                <img src="svg/upload.svg" alt="Unggah Berkas" style="width: 20px; height: 20px; margin-right: 10px;">
                <span>Unggah Berkas</span>
                <input id="file-upload" type="file" name="berkas">
            </label>
            <div id="upload-success" style="display: none;" class="upload-message">
                <p>Berkas berhasil diunggah!</p>
            </div>
        </div>
...
document.getElementById('job_description').addEventListener('input', function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Ambil input file dan tambahkan event listener untuk perubahan nilai (saat berkas diunggah)
        var inputFile = document.getElementById('file-upload');
        inputFile.addEventListener('change', function() {
            var uploadMessage = document.getElementById('upload-success');
            uploadMessage.style.display = 'block'; // Tampilkan pesan upload
            setTimeout(function() {
                uploadMessage.style.display = 'none'; // Sembunyikan pesan setelah beberapa detik
            }, 3000); // Terserah Anda untuk mengatur durasi tampilan pesan
        });
    });
